        ...
        {
            /*for(vector<const HashedObj*>::iterator itr = array.begin(); itr != array.end(); itr++)
                delete (*itr);
            array.clear();
        ...
        void QuadraticPtrHashTable<HashedObj>::insert( const HashedObj & x )
        {
            ...
            if( isActive( currentPos ) )
                return;
            array[ currentPos ] = new HashedObj(x); //the item inserted
            //initialized eveything to NULL
            //lazy deletion is ITEM_NOT_FOUND
        ...
        {
            vector<const HashedObj*> oldArray;
...
/* 3*/      while( array[ currentPos ] != NULL && //EMPTY
                   *array[ currentPos ] != x )
            {
